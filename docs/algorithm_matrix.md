...
| Uncertainty-reduction forecasting | random forest + leave-one-out jackknife+ + stable delay timer | `UncertaintyReductionForecaster` | B | official-v3 40-task author grid plus 120 split/randomness controls complete; paper rows pass 6/16 author and 8/16 seeded-disjoint; exact numeric, CASIM Numba RNG, Docker parity, and independent same-fold gates remain |
