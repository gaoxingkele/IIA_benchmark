...
| ConE-AFC | class- and stepwise inductive conformal prediction | independent callable wrapper + official Code Ocean comparison slot | B | official 250-task/50-split grid closes 95/95 Tables I-II means within 0.02 (maximum delta 0.002271); independent conformal layer matches 1,800/1,800 MBW-LR same-fold metrics exactly; Docker and independent base-model/end-to-end gates remain |
...
| Uncertainty-reduction forecasting | random forest + leave-one-out jackknife+ + stable delay timer | `UncertaintyReductionForecaster` | B | official-v3 40-task author grid plus 140 split/randomness controls complete; paper rows pass 6/16 author and 8/16 seeded-disjoint; explicit Numba seeding closes all 10 CASIM paired-bifurcation controls, while exact numeric, Docker parity, and independent same-fold gates remain |
