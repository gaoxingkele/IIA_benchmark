...
| `fcc_alarm` | ReSeeD 官方 4 个附件已下载；2 个 ZIP 的 SHA-256 与全量 CRC 通过，2 个 PDF 已校验；adapter/实验待实现 | 1,600 个仿真 run、16 类异常、每类 100 run；每个 run 有 60 个一分钟采样、57 个报警位，并配套过程/阀位/扰动序列；T3/T4/open-set/鲁棒性 | [ReSeeD](https://reseed.ruhr-uni-bochum.de/concern/datasets/19217e41-e9ad-4c0e-bb51-97e7072813f7)，FCC DOI `10.60517/2v23vv393`，CC BY 4.0；误命名候选包实际 DOI `10.60517/19027803-fec9-41f2-8a02-408cc176554e` |
...
DataPort 的二进制归档不能通过绕过登录/条款的方式抓取。当前 TEP/NPP 原始载荷已通过认证传输取得并补齐 checksum；FCC Alarm 已从公开的 ReSeeD 官方记录直接取得。三者在 adapter、grouped split 和论文参考分数跑通前，算法成熟度仍不得提升为 A。
