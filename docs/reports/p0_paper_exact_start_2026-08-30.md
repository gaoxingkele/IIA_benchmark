...
ConE-AFC 作者 v2 默认复跑也已完成。默认条件为五折、`alpha=0.05`、每类使用 22 个校准样本；10 个可直接对应的 Table 1/2 行中 9 行落在预设 ±0.02 容差内。CASIM+ConE 的 coverage 为 `0.912130`（论文 `0.914`），平均集合大小为 `1.636129`（论文 `1.640`）。唯一超差项是 EAC-1NN 的平均集合大小：`1.903470` 对论文 `1.879`，差 `+0.024470`。由于论文使用 10×5=50 个测试，而默认 Capsule 仅运行 5 个测试，这一结果仍记为 P2，不能将 9/10 的数值接近误报为 P3。
...
| ConE-AFC | `10.24433/CO.5512337.v2` | `198fa6ee…cf48e8a6` | 12 / 18,751 files | 作者默认复跑完成；9/10 对照行通过 |
| BiP-AFC | `10.24433/CO.3008979.v3` | `3e33382e…8d25b3f` | 11 / 2,876 files | 作者默认复跑进行中 |
